...
void loop()
{
  ...
  //data.yaw = constrain(data.yaw, -45.0, 45.0); 
;
  data.AUX1 = arm;
  //Serial.print(data.yaw);
  //Serial.print("\t");
/*int k=map(data.roll,   -45,   45, 1000, 2000);
  if(k<1500&&millis()<15000){
     digitalWrite(9, HIGH);
  }
  else {
    digitalWrite(9, LOW);
  ...
  int n=0;
  if((ypr[0] * 180/M_PI)-n >0){
      n-=(ypr[0] * 180/M_PI);
      n=-n;
   //   double *k=(ypr[0] * 180/M_PI);;
      //n-=k;
      }
  else
      n= 0;  
  int l=map(n,0,179,1000,2000) ;
  if(l>1500&&l<1520) {
